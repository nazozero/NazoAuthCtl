...
fn controller_state_directory() -> anyhow::Result<PathBuf> {
    let store = DeploymentStore::system();
    store.validate_failure_domains()?;
    Ok(store.state_root.join("controller-self"))
}

fn controller_check(version: Option<&str>) -> anyhow::Result<()> {
...
fn controller_update(version: Option<&str>) -> anyhow::Result<()> {
    let store = DeploymentStore::system();
...
fn controller_rollback() -> anyhow::Result<()> {
    let store = DeploymentStore::system();
    let _lock = store.controller_self_lock()?;
    verify_controller_self_audit()?;
    controller_self_audit_signer()?;
...
fn controller_trust_state() -> anyhow::Result<Option<ControllerTrustState>> {
...
fn enforce_controller_trust(version: &str, sha256: &str) -> anyhow::Result<()> {
...
fn write_controller_trust(version: &str, sha256: &str) -> anyhow::Result<()> {
    let directory = controller_state_directory()?;
    fs::create_dir_all(&directory)?;
    atomic_write(
...
fn controller_install_path(current: &Path) -> anyhow::Result<PathBuf> {
...
fn controller_self_audit_directory() -> anyhow::Result<PathBuf> {
    Ok(controller_state_directory()?.join("audit"))
}

fn controller_self_audit_signer() -> anyhow::Result<(String, SigningKey)> {
...
fn verify_controller_self_audit() -> anyhow::Result<(u64, String)> {
...
fn append_controller_self_audit(
    operation: &str,
    from_version: &str,
    to_version: &str,
    artifact_sha256: &str,
