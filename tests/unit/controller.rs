...
#[test]
fn oidf_run_locks_overlap_and_exclude_lifecycle_mutation() {
    let work = PrivateTempDir::new("nazoauth-oidf-run-lock").unwrap();
    let path = work.path().join("lifecycle.lock");

    let run_a = acquire_oidf_run_shared_lock_at(&path).unwrap();
