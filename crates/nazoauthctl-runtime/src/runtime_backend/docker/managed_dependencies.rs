...
pub(super) fn backup(command: &OsStr, backup: &ManagedDependencyBackup) -> anyhow::Result<()> {
    container_shared::require_digest_pinned_image(&backup.postgres_validation_image, "Docker")?;
...
pub(super) fn restore_postgres(
    ...
    container_shared::require_digest_pinned_image(&restore.image, "Docker")?;
    container_shared::assert_managed_labels(
        command,
    ...
    {
        bail!("Docker managed restore journal is not ready for PostgreSQL restore");
    }
    ...
    {
        bail!("Docker managed restore journal contains unsafe PostgreSQL database names");
    }
    ...
    if journal.phase == "postgres-prepared" {
        ...
        if !original_exists && quarantine_exists {
            ...
            bail!("Docker PostgreSQL restore journal and database state disagree");
        }
    }
    if journal.phase == "postgres-old-quarantined" {
        ...
        if temporary_exists && !original_exists {
            ...
            bail!("Docker PostgreSQL restore journal and temporary database state disagree");
        }
    }
    Ok(())
}
...
pub(super) fn restore_valkey(
    ...
    if !matches!(
        ...
        bail!("Docker managed restore journal is not ready for Valkey restore");
    }
...
fn ensure_restore_volume(
    ...
    if container_shared::inspect_document_optional(command, &arguments, "Docker")?.is_some() {
...
fn validate_temporary_valkey(
    ...
    let container_id = match container_shared::inspect_managed_container_id(
        ...
        Ok(None) => bail!("temporary Docker Valkey restore disappeared before validation"),
        Err(error) => {
            ...
            if let Err(cleanup) = cleanup {
                return Err(error.context(format!(
                    "temporary Docker Valkey restore cleanup failed: {cleanup}"
                )));
            }
            return Err(error);
        }
...
fn copy_valkey_volume(
    ...
    .arg("--volume")
    .arg(format!("{destination}:/destination"))
    .arg(&restore.image)
    .args([
        "sh",
        "-eu",
...
pub(super) fn ensure_network(
    ...
    if container_shared::inspect_document_optional(command, &network_inspect, "Docker")?.is_some() {
        container_shared::assert_managed_labels(
            command,
            &network_inspect,
...
pub(super) fn ensure_dependencies(
    ...
    container_shared::require_digest_pinned_image(&dependencies.postgres_image, "Docker")?;
    container_shared::require_digest_pinned_image(&dependencies.valkey_image, "Docker")?;
...
fn postgres_database_exists(
    ...
    match output.trim() {
        "" => Ok(false),
        "1" => Ok(true),
        _ => bail!("Docker PostgreSQL database existence probe returned invalid output"),
    }
}

fn ensure_container_started(command: &OsStr, object_reference: &str) -> anyhow::Result<()> {
    if container_shared::container_is_running(command, object_reference, "Docker")? {
        return Ok(());
    }
    Process::new(command)
